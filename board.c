...
{
	// Checking the values stored in board by finding values and storing in temporary then checking if they match with what is present in the actual board
	...
	ASSERT(GeneratePosKey(pos)==pos->posKey);			// ( Not sure about this yet )
...
int ParseFen(char *fen,S_BOARD *pos)  // Parsing the fen given from gui or our board
{
	ASSERT(fen!=NULL);
	ASSERT(pos!=NULL);
